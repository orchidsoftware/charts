...
describe("performance budgets in Chromium", () => {
  ...
  it("renders a 50,000-point line in under one second", () => {
    ...
    expect(document.querySelector("path").getAttribute("d").length).toBeGreaterThan(1_000_000);
  ...
  it("handles 200 live updates of 100 points in under one second", () => {
    ...
    const duration = measure(() => {
      for (let iteration = 0; iteration < 200; iteration += 1) {
        chart.update({ datasets: [{ values: Array.from({ length: 100 }, (_, index) => index + iteration) }] });
      }
    });
    expect(duration).toBeLessThan(1000);
  });
