...
describe("internal boundary validation", () => {
  ...
  ])("rejects invalid runtime options: %j (%s)", (options, message) => {
  ...
  it("validates only color-bearing fields declared by public data schemas", () => {
    ...
    expect(() =>
      validateChartColors(host, {
        colors: ["red"],
        data: {
          datasets: [null, [], { values: unreadableValues }, { values: unreadableValues, color: "blue" }],
          markers: [{ color: "green", labelColor: "purple" }],
          regions: [{ color: "orange", labelColor: "black" }],
          tasks: [{ color: "white" }],
        },
      }),
    ).not.toThrow();
    ...
    for (const input of [
      {
        colors: ["definitely-not-a-color"],
      },
      {
        datasets: [{ color: "definitely-not-a-color" }],
      },
      {
        markers: [{ labelColor: "definitely-not-a-color" }],
      },
      {
        regions: [{ color: "definitely-not-a-color" }],
      },
      {
        tasks: [{ color: "definitely-not-a-color" }],
      },
    ]) {
